...
def find_communityOrder(collapsed_AdjList): # Here G is a collapsed graph version of the original graph, and is weighted and directed.
    ...
    while number_of_passes < (20 * n): # Here, we keep the max passes as 5n and not nC2 because the number of 
...
#             print("}")
    ...
    fractional_violations = [0 for i in range(1)]
    total_between_edges = [0 for i in range(1)]
    directionalities = [0 for i in range(1)]
    node_division = []
    run = 0
    ...
    while run < 1:
        ...
        while alpha <= 0.6:
            ...
            b_rs, B_matrix = brs_matrix(AdjList, final_answer, rho, flag_model, number_of_communities, degree_in, degree_out)  # b_rs matrix to start with
    #print(b_rs)
            detected_R = calculate_rankularity(final_answer, b_rs, alpha)
...
#                 print("Node counts = ", node_count)
    ...
    plt.show()
#     print("node_division = ", node_division)
    return node_division, total_between_edges, fractional_violations, directionalities, alphawise_information, detected_R
...
def get_Rankularity_results(realWorldNetwork, number_of_communities):
    ...
    detected_R = rankularity_results[-1]
    
    return graph_and_hashes,node_division,alphawise_info, detected_R