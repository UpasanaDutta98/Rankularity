...
def run_SBM(AdjList, list_of_numberOfNodes):
    g = Graph() # By default, it is dircted
    numnodes = len(AdjList)
    vlist = g.add_vertex(numnodes) 
    
    for n1 in range(len(AdjList)):
        for n2 in AdjList[n1]:
            g.add_edge(g.vertex(n1), g.vertex(n2))
    ...
    state = minimize_blockmodel_dl(g) # Degree-corrected SBM.
    b = state.get_blocks()
    best_partition = []
    for v in g.vertices():
        best_partition.append(b[v])
    
    number_of_communities = max(best_partition)+1
    
    if number_of_communities == 1:
        final_answer = best_partition
    else:
    ...
    fractionCorrect1 = fraction_of_correct_nodes(ground_truth, best_partition) # Think on this, is this correct?
    NMI1 = normalized_mutual_info_score(ground_truth, best_partition) 
    coef_kt1, pvalue_kt1 = kendalltau(ground_truth, best_partition) 

    fractionCorrect2 = fraction_of_correct_nodes(ground_truth, final_answer) # Think on this, is this correct?
    NMI2 = normalized_mutual_info_score(ground_truth, final_answer) 
    coef_kt2, pvalue_kt2 = kendalltau(ground_truth, final_answer) 
    return fractionCorrect1,coef_kt1,NMI1,fractionCorrect2,coef_kt2,NMI2
...
#     ground_truth_Rankularity = best_R
    ...
    b_rs, B_matrix = brs_matrix(AdjList, best_partition, rho, flag_model, c, degree_in, degree_out)  # b_rs matrix to start with
    best_R_found = calculate_rankularity(best_partition, b_rs, alpha)  # initial Rankularity score
...
#         same_order_as_MVR = 0
    ...
    print("Completed comm = ", c)  
    #print("alpha = ", alpha, "Spearman = ", coef_sp, pvalue_sp)
    return fractionCorrect,coef_kt,best_R_found,NMI, coef_sp,coef_ps,SBM_matrix,list_of_numberOfNodes,confusion, NMI,best_partition, coef_ps, pvalue_ps, coef_sp, pvalue_sp, coef_kt, pvalue_kt
...
def get_SBM_matrix(k, b):
    # Same matrix will be used for generating the graph for Rankularity vs SBM conversion because it would be the same network
    SBM_matrix = [[0 for com in range(k)] for com in range(k)]
    for i in range(k):
        # Upper triangular elements of the row
        for j in range(i+1, k):
            b_val = np.random.normal(b, 0.01, None)
            SBM_matrix[i][j] = round(b_val, 3)

        # Diagonal element of the row
        a_val = np.random.normal(0.05, 0.01, None)
        SBM_matrix[i][i] = round(a_val,3)

        # Lower triangular elements of the row
        j = 0
        while j < i:
            c = np.random.normal(0.005, 0.001, None)
...
def get_performance_varyingDominance(iteration, k, return_dict):
    Accuracies_R = []
    Kendalltau_R = []
    RankularityScore_R = []
    NMI_R = []
    ...
    # k = ground truth number of communities here
            
    alpha = 0.6
    start = 1            
    b = 0.01
    while b <= 0.2:
        list_of_numberOfNodes = [random.randint(30, 80) for comsize in range(k)] # For K math, all communities are of same size.
        SBM_matrix = get_SBM_matrix(k, b)
        ...
        print("b = ", b, ", iteration = ", iteration, ", alpha = ", alpha)
        returned_R = run_Rankularity(start,AdjList, rho, degree_in, degree_out, list_of_numberOfNodes,alpha,k,1,SBM_matrix)
        
        Accuracies_R.append(returned_R[0])
        Kendalltau_R.append(returned_R[1])
        RankularityScore_R.append(returned_R[2])
        NMI_R.append(returned_R[3])

        returned_SBM = run_SBM(AdjList, list_of_numberOfNodes)
        
        Accuracies_SBM.append(returned_SBM[0])
        Kendalltau_SBM.append(returned_SBM[1])
        NMI_SBM.append(returned_SBM[2])
        Accuracies_SBM_MVR.append(returned_SBM[3])
        Kendalltau_SBM_MVR.append(returned_SBM[4])
        NMI_SBM_MVR.append(returned_SBM[5])

        b = b + 0.01
        b = round(b, 2)
        
    return_dict[iteration] = [Accuracies_R, Kendalltau_R, RankularityScore_R, NMI_R, Accuracies_SBM, Kendalltau_SBM, NMI_SBM, Accuracies_SBM_MVR, Kendalltau_SBM_MVR, NMI_SBM_MVR]

    

def get_PerformanceComparison(k):
    ...
    iterations = 0
    while iterations < 30:
        p = multiprocessing.Process(target=get_performance_varyingDominance, args=(iterations, k, return_dict))
        jobs.append(p)
        p.start()
        iterations += 1
...
def final_call(k):
    t1 = time.time()
    returned_values = get_PerformanceComparison(k)
    pickle_out = open("PerformanceComparison_with"+str(k)+"_withCI.pkl","wb")
