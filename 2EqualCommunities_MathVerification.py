...
def get_SBM_matrix(a, b):
    SBM_matrix = [[0 for com in range(2)] for com in range(2)]
    for i in range(2):
        print("i = ", i, end = " ")
        # Upper triangular elements of the row
        for j in range(i+1, 2):
            SBM_matrix[i][j] = round(np.random.normal(b, 0.1, None),3)

        # Diagonal element of the row
        SBM_matrix[i][i] = round(np.random.normal(a, 0.1, None),3)

        # Lower triangular elements of the row
        j = 0
        while j < i:
            c = round(np.random.normal(0.1, 0.05, None),3)
    ...
    print("Built SBM matrix for k = ", 2)
    return SBM_matrix
...
def get_performance_varyingAlpha(iteration, a, b, return_dict):
    ...
    order_flag = 0
    while order_flag == 0:
        nodes = random.randint(10, 200)
        list_of_numberOfNodes = [nodes for comsize in range(2)] # For K math, all communities are of same size.
        SBM_matrix = get_SBM_matrix(a, b)
    ...
    while alpha <= 1: # I have to find out why moving alpha here is faster
...
def final_call(a, b):
    ...
    pickle_out = open("2EqComm_"+str(a)+"_"+str(b)+"_withCI.pkl","wb")
