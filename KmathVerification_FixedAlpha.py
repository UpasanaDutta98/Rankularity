...
def get_performance_varyingK(iteration, return_dict):
    Accuracies_06 = []
    Kendalltau_06 = []
    k = 2
    alpha = 0.6
...
def get_RankularityPerformance():
    ...
    SBM_information_00 = []
    iterations = 0
    while iterations < 50: # The CI is calculated over 50 runs
        p = multiprocessing.Process(target=get_performance_varyingK, args=(iterations, return_dict))
        jobs.append(p)
        p.start()
        iterations += 1
...
def final_call():
    t1 = time.time()
    returned_values = get_RankularityPerformance()
    pickle_out = open("KmathVerification_alpha06_withCI.pkl","wb")
